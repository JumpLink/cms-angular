...
.service('BlogBootstrapService', function (moment, UtilityService, $sailsSocket, $async, $log, focus, $modal, FileUploader) {
  ...
  var setModals = function($scope, fileOptions) {
    ...
    editModal.$scope.$on('modal.hide.before',function(blogPost, editModal) {
      ...
      if(editModal.$scope.ok) {
        return validate(editModal.$scope.blogPost, null, editModal.$scope.callback);
      } else {
